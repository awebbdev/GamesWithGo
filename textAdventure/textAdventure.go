...
func main(){
	...
	grue := storyNode{text: "While stumbling around in the darkness, you are eatren by a grue."}
	trap := storyNode{text: "You head down the well traveled path when suddenly a trap door opens and you fall in to a pit"}
	treasure := storyNode{text: "You arrive in a asmall chamber, filled with treasure"}

	start.addChoice("N", "Go North", &darkRoom)
	start.addChoice("S", "Go", &darkRoom)
